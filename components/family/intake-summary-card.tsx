  ...
  return (
    <div className="space-y-4">
      ...
      <article className={`rounded-2xl border border-stone-200 bg-white shadow-soft ${compact ? "p-4" : "p-5 sm:p-6"}`}>
        ...
        {intake.decisionMakerRelationship ? (
          ...
          {intake.mobility ? (
            <div>
              ...
              <dd className="font-medium text-neutral-800">{intake.mobility}</dd>
            </div>
          ) : null}
          {intake.dementiaNeeds ? (
            <div>
              <dt className="text-neutral-500">Dementia / memory</dt>
              <dd className="font-medium text-neutral-800">{intake.dementiaNeeds}</dd>
            </div>
          ) : null}
        </dl>

        <p className="mt-4 text-sm leading-6 text-neutral-600">{hint}</p>
        <p className="mt-2 text-xs italic text-neutral-500">{ubuntuTagline}</p>
        ...
        {visitSummary ? (
          <p className="mt-3 rounded-xl border border-brand-amber/30 bg-brand-amber/10 px-4 py-3 text-sm leading-6 text-neutral-800">
